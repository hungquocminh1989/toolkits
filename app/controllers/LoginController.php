...
class LoginController extends ControllerBase
{
    ...
    public function authAction()
    {
        ...
        try{
			...
			$this->createLog($this->getDefine()->LOGGING->SCREEN_LOGIN,'Thông tin đăng nhập',$params);
            $modelUser = new User();
            ...
            if($m_user_id !== FALSE){
                ...
                $this->createLog($this->getDefine()->LOGGING->SCREEN_LOGIN,'Đăng nhập thành công.');
                return json_encode($arrResponse);
            }
            else{
                $arrResponse['error_msg'] = $this->getDefine()->MESSAGES->HTML_MSG_ERROR_LOGIN_FAIL;
                $this->createLog($this->getDefine()->LOGGING->SCREEN_LOGIN,'Đăng nhập thất bại.');
                return json_encode($arrResponse);
			}
        } catch (Exception $ex) {
            $arrResponse['error_msg'] = $ex->getMessage();
            $this->createLog($this->getDefine()->LOGGING->SCREEN_LOGIN,'Đăng nhập có lỗi xảy ra.');
            return json_encode($arrResponse);
        }
    }
